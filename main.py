...
def plot_variable_importance(Log_Reg, X_train):
    imp=df({"imp":Log_Reg.feature_importances_, "names":X_train.columns}).sort_values("imp", ascending=True)
...
row2_col1.pyplot(plot_variable_importance(Log_Reg, X_train), use_container_width = True)
