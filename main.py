...
def plot_variable_importance(model, X_test):
    imp=df({"imp":model.feature_importances_, "names":X_test.columns}).sort_values("imp", ascending=True)
...
row2_col1.pyplot(plot_variable_importance(model, X_test), use_container_width = True)
